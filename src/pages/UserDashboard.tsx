...
const UserDashboard: React.FC = () => {
  ...
  const handleNewRequest = (e: React.FormEvent) => {
    ...
    const newRequestId = bloodRequests.length > 0 ? Math.max(...bloodRequests.map(req => req.id)) + 1 : 1;
    ...
    <div className="container mt-4">
      ...
      <div className="card mb-4">
        ...
        <div className="card-body">
          ...
          <p><strong>Blood Type:</strong> <span className="badge bg-danger fs-6">{loggedInUser.bloodType}</span></p>
      ...
      <div className="card mb-4">
        ...
        <div className="card-body">
          <form onSubmit={handleNewRequest}>
            <div className="mb-3">
              <label htmlFor="bloodTypeSelect" className="form-label">Blood Type Needed</label>
              <select
                id="bloodTypeSelect"
      ...
      <div className="row">
        {bloodRequests.length > 0 ? (
            ...
            <div className="col-md-6 col-lg-4 mb-4" key={request.id}>
              <div className="card h-100">
                <div className="card-body d-flex flex-column">
                  <h5 className="card-title text-danger">Blood Type: {request.bloodType}</h5>
                  <p className="card-text"><strong>Requester:</strong> {request.requesterName}</p>
                  <p className="card-text"><strong>Status:</strong> <span className="badge bg-warning text-dark">{request.status}</span></p>
                  <p className="card-text"><small className="text-muted">Requested on: {new Date(request.createdAt).toLocaleDateString()}</small></p>
                  {/* In a real app, a user might "volunteer" here */}
                  <button className="btn btn-outline-danger mt-auto" disabled>Volunteer (Coming Soon)</button>
                </div>
              </div>
            </div>
          ))
          ...
          <div className="col-12">
            <p className="text-muted">No active blood donation requests at the moment.</p>
          </div>
        )}
      </div>
    </div>
